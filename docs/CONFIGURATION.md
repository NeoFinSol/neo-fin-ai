...
| `TASK_QUEUE_RESULT_BACKEND` | `string` | — | — | Адрес Redis-хранилища результатов Celery. В текущем контуре используется как часть согласованной конфигурации worker-процесса |
| `TASK_EVENTS_REDIS_URL` | `string` | — | — | Адрес Redis-канала для переноса событий статуса из worker-процесса в WebSocket-процесс FastAPI |
| `TASK_QUEUE_NAME` | `string` | `neofin` | — | Имя очереди задач для worker-процесса |
| `TASK_QUEUE_EAGER` | `bool` | `false` | — | Режим eager-выполнения Celery; нужен только для специальных локальных сценариев и отладки |

Поведение:
- `TASK_RUNTIME=background` сохраняет локальный безопасный путь без внешней инфраструктуры
- `TASK_RUNTIME=celery` требует рабочий Redis и отдельный worker-процесс
