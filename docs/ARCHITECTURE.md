...
**НеоФин.Документы** — первый модуль экосистемы **НеоФин.Контур**, гибридная платформа финансового анализа предприятий на основе PDF-отчётности.
...
- Расчёт 15 коэффициентов в публичном API, из которых 13 участвуют в интегральном скоринге, с нормализацией по benchmark-профилям `generic` и `retail_demo`
- Интегральный скоринг 0–100 с уровнем риска, факторами влияния и нормализованными значениями
- Возврат explainability-контракта `score.methodology` и truthful AI-статуса `ai_runtime`
...
Такой подход выбран не только из инженерных соображений. Он напрямую поддерживает продуктовые требования НеоФин.Документы:
...
│                                                      │
│  • Расчет 15 коэффициентов в публичном API           │
│  • 13 коэффициентов участвуют в интегральном score   │
│  • Нормализация по benchmark-профилям                │
│  • Интегральный скоринг 0–100                        │
│  • Расчёт confidence_score и score.methodology       │
│                                                      │
...
НеоФин.Документы — не конвейер, в котором языковая модель просто стоит в конце. Это **двухуровневая система**, где каждый уровень решает отдельную задачу и может работать независимо.
...
| `ratios.py` | Расчёт 15 коэффициентов в публичном API, из них 13 — score-active | Прозрачная математика |
| `scoring.py` | Нормализация по benchmark-профилям `generic` и `retail_demo`, скоринг 0–100 | Объяснимые веса и пороги |
...
НеоФин.Документы отвечает на эти вопросы явно на каждом шаге.
...
- `score.methodology` — benchmark-профиль, база периода, guardrails, leverage-basis, peer-context
- `ai_runtime` — requested/effective provider и честный статус выполнения AI-контура
...
- **Сводная строка**: «Надёжно извлечено: N показателей»
...
### Truthful scoring и benchmark-aware интерпретация

Текущий scoring-контур публично фиксирует только два benchmark-профиля: `generic` и `retail_demo`. Для квартальных и полугодовых документов применяется `period_basis` (`reported`, `annualized_q1`, `annualized_h1`), а для lease-heavy retail используется explainable логика dual leverage:

- `financial_leverage_total`
- `financial_leverage_debt_only`
- `financial_leverage` как backward-compatible alias для display/scoring

Это позволяет не только посчитать итоговый score, но и объяснить, по какой методике он получен и какие guardrails или issuer overrides повлияли на результат.
...
- **score.methodology** — видно, по какому benchmark’у и period-basis рассчитан итог
- **ai_runtime** — интерфейс честно показывает, был ли AI вызван и чем это закончилось
...
- Архитектура поддерживает главный публичный тезис НеоФин.Документы: продукт не просто генерирует вывод, а даёт управляемый, проверяемый и воспроизводимый результат
