...
| Продакшн | `http://<host>` (порт 80, через Nginx в роли обратного прокси) |

### Формат содержимого

| Операция | Тип содержимого |
|---|---|
...
- `confidence < 0.5` → показатель исключается из расчётов, но остаётся видимым в интерфейсе
...
Уведомления в реальном времени о статусе выполнения задачи. Рекомендуется использовать вместо периодического опроса для мгновенного обновления интерфейса.
...
3.  **ИИ-анализ**:
    ```json
    {"status": "analyzing", "message": "Генерация выводов через ИИ..."}
    ```
4.  **Завершение**:
    ```json
    {
...
**Преимущества**:
- Мгновенное отображение прогресса.
- Снижение нагрузки на базу данных за счёт отсутствия частых запросов.
...
Полные данные анализа по `task_id`. Эндпоинт возвращает внутреннюю полезную нагрузку анализа из `result.data`, без внешней обёртки `filename`/`data`.
...
Расширенная проверка с верификацией зависимостей: подключение к базе данных и статус ИИ-сервиса.
