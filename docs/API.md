...
| Production | `http://<host>` (порт 80, через Nginx reverse proxy) |
...
### Источники данных и Confidence

| Источник | Confidence | Описание |
|----------|------------|----------|
...
- Порог настраивается: `CONFIDENCE_THRESHOLD=0.5` (env variable)
...
| `data.extraction_metadata` | object | Confidence Score и источник для каждого показателя (см. раздел 3) |
...
Значение `null` означает, что показатель не найден в документе или его Confidence Score ниже порога `CONFIDENCE_THRESHOLD` (по умолчанию 0.5).
...
### Источники извлечения (ExtractionSource)

| Значение | Confidence | Описание |
|---|---|---|
| `table_exact` | 0.9 | Точное совпадение ключевого слова в таблице |
