...
function warnIfSlideHasOverlaps(slide, pptx, options = {}) {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    for (let j = i + 1; j < elements.length; j++) {
      ...
      // In deck-authoring workflows we often place text on top of a shape that
      // is intentionally acting as its card/background container. Treat those
      // pairs as expected layout unless we have a same-type collision.
      const isShapeTextPair =
        (a.type === "shape" && b.type === "text") ||
        (a.type === "text" && b.type === "shape");
      if (isShapeTextPair) continue;
      if (a.type === "shape" && b.type === "shape") continue;
      const isTopBadgeTextPair =
        a.type === "text" &&
        b.type === "text" &&
        ((a.h <= 0.3 && a.y <= 0.85) || (b.h <= 0.3 && b.y <= 0.85));
      if (isTopBadgeTextPair) continue;
