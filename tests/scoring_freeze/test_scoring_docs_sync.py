...
def test_inventory_doc_is_renderer_synced() -> None:
    ...
    committed = (_ROOT / "docs" / "scoring_freeze_inventory.md").read_text(
        encoding="utf-8"
    ).strip()
...
def test_classification_doc_is_renderer_synced() -> None:
    ...
    committed = (_ROOT / "docs" / "scoring_freeze_classification.md").read_text(
        encoding="utf-8"
    ).strip()
    assert committed == expected
...
def test_payload_matrix_doc_is_renderer_synced() -> None:
    ...
    committed = (_ROOT / "docs" / "scoring_freeze_payload_matrix.md").read_text(
        encoding="utf-8"
    ).strip()
    assert committed == expected
