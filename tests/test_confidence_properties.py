...
from src.tasks import _apply_confidence_filter, CONFIDENCE_THRESHOLD
...
# Valid extraction sources (for direct use)
VALID_SOURCES = st.sampled_from(["table_exact", "table_partial", "text_regex", "derived"])
...
# Expected confidence values for each source
EXPECTED_CONFIDENCE = {
    "table_exact": 0.9,
    "table_partial": 0.7,
    "text_regex": 0.5,
...
def test_confidence_score_in_range(match_params):
    ...
    _, confidence = determine_source(match_type, is_exact=is_exact, is_derived=is_derived)
...
def test_source_confidence_mapping(match_params):
    ...
    """
    match_type, is_exact, is_derived = match_params
    returned_source, confidence = determine_source(match_type, is_exact=is_exact, is_derived=is_derived)
    
    # Determine expected source from params
    if is_derived:
        expected_source = "derived"
    elif match_type == "table":
        expected_source = "table_exact" if is_exact else "table_partial"
    elif match_type == "text_regex":
        expected_source = "text_regex"
    else:
        expected_source = "derived"
    
    # Source should be returned correctly
    assert returned_source == expected_source, f"Expected {expected_source}, got {returned_source}"
    
    # Confidence should match expected value exactly
    assert confidence == EXPECTED_CONFIDENCE[returned_source], \
        f"Expected {EXPECTED_CONFIDENCE[returned_source]} for {returned_source}, got {confidence}"
...
def test_metadata_structure_always_valid(metric_data):
    ...
    for key, meta in extraction_metadata.items():
        ...
        assert meta["source"] in ["table_exact", "table_partial", "text_regex", "derived"]
...
def test_edge_case_thresholds(threshold):
    ...
    if threshold == 0.0:
        ...
        # None should pass (max confidence is 0.9)
...
@given(
    st.dictionaries(
        keys=st.sampled_from([
            ...
            "liabilities", "current_assets", "accounts_receivable"
        ]),
...
def test_valid_pdf_metrics_dont_crash(metric_values):
    ...
    for i, (key, value) in enumerate(metric_values.items()):
        ...
        _, confidence = determine_source(source)
        metadata[key] = ExtractionMetadata(value=value, confidence=confidence, source=source)
