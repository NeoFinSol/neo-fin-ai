...
class TestUploadPdf:
    ...
    @pytest.mark.asyncio
    async def test_successful_upload_uses_shared_storage_dir_in_celery_runtime(
        self, tmp_path
    ):
    ...
    async def test_dispatch_failure_marks_analysis_failed_and_cleans_temp_file(self):
        ...
        with patch("src.routers.pdf_tasks.create_analysis", new_callable=AsyncMock):
            with patch(
                "src.routers.pdf_tasks.update_analysis", new_callable=AsyncMock
            ) as mock_update:
                with patch(
    ...
    @pytest.mark.asyncio
    async def test_temp_file_close_errors_do_not_abort_upload(self):
        """Best-effort temp file close should not break a successful upload."""
        mock_background = MagicMock(spec=BackgroundTasks)
        mock_file = MagicMock(spec=UploadFile)
        mock_file.content_type = "application/pdf"
        mock_file.filename = "test.pdf"
        ...
        with patch("src.routers.pdf_tasks.tempfile.NamedTemporaryFile") as mock_temp:
            ...
            with patch(
                "src.routers.pdf_tasks.create_analysis",
    ...
    @pytest.mark.asyncio
    async def test_invalid_ai_provider_cleans_saved_temp_file(self):
        """Provider validation failures should not leak saved temp files."""
        mock_background = MagicMock(spec=BackgroundTasks)
        mock_file = MagicMock(spec=UploadFile)
        mock_file.content_type = "application/pdf"
        mock_file.filename = "test.pdf"
        ...
        with patch("src.routers.pdf_tasks.tempfile.NamedTemporaryFile") as mock_temp:
            ...
            mock_temp.return_value = mock_temp_instance
            with patch(
                "src.routers.pdf_tasks._cleanup_temp_file",
                new_callable=AsyncMock,
            ) as mock_cleanup:
                with patch(
                    "src.routers.pdf_tasks.ai_service.normalize_requested_provider",
                    return_value="ollama",
                ):
                    with patch.object(
                        type(
...
class TestGetResult:
    ...
    async def test_task_not_found(self):
        ...
        with patch(
            "src.routers.pdf_tasks.get_analysis", new_callable=AsyncMock
        ) as mock_get:
    ...
    async def test_task_found_with_non_dict_result(self):
        ...
        with patch(
            ...
            assert (
                "result" not in result or result.get("result") != "some string result"
            )
...
class TestCancelAnalysis:
    ...
    async def test_cancel_analysis_requests_cancellation(self):
        ...
        with patch(
            ...
            mock_get.return_value = mock_analysis
            with patch(
                "src.routers.pdf_tasks.request_analysis_cancellation",
                new_callable=AsyncMock,
            ) as mock_request:
