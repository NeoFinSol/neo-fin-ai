...
class TestConfidenceMapping:
    """Test exact V2 baseline confidence values for each extraction profile."""

    def test_table_exact_confidence_is_0_92(self):
        """table exact profile should map to table/0.92."""
...
class TestUnknownSourceFallback:
    ...
    def test_unknown_source_falls_back_to_derived(self):
        """Unknown source should fallback to derived with V2 baseline 0.35."""
...
class TestConfidenceThresholdFiltering:
        ...
        filtered, metadata = _apply_confidence_filter(
            {
                "revenue": ExtractionMetadata(
                    value=1000.0, confidence=0.9, source="table_exact"
                )
            }
        )
    ...
    def test_value_at_threshold_passes(self):
        ...
        filtered, metadata = _apply_confidence_filter(
            {
                "revenue": ExtractionMetadata(
                    value=1000.0, confidence=0.5, source="text_regex"
                )
            }
        )
        assert filtered["revenue"] == 1000.0
    ...
    def test_value_below_threshold_is_none(self):
        ...
        filtered, metadata = _apply_confidence_filter(
            {
                "revenue": ExtractionMetadata(
                    value=1000.0, confidence=0.3, source="derived"
                )
            }
        )
    ...
    def test_value_with_confidence_0_7_passes(self):
        ...
        filtered, metadata = _apply_confidence_filter(
            {
                "revenue": ExtractionMetadata(
                    value=1000.0, confidence=0.7, source="table_partial"
                )
            }
        )
        assert filtered["revenue"] == 1000.0
...
class TestDictionaryFiltering:
    ...
    def test_all_values_above_threshold(self):
        ...
        metadata = {
            ...
            "profit": ExtractionMetadata(
                value=100.0, confidence=0.7, source="table_partial"
            ),
        }
    ...
    def test_none_value_always_preserved(self):
        ...
        metadata = {
            "revenue": ExtractionMetadata(
                value=None, confidence=0.9, source="table_exact"
            ),
        }
...
class TestMetadataPreservation:
    ...
    def test_metadata_preserved_for_passed_values(self):
        ...
        assert extraction_metadata["revenue"]["source"] == "table"
        assert extraction_metadata["revenue"]["match_semantics"] == "exact"
    ...
    def test_all_keys_present_in_metadata(self):
        ...
        metadata = {
            ...
            "profit": ExtractionMetadata(value=100.0, confidence=0.3, source="derived"),
            "assets": ExtractionMetadata(
                value=5000.0, confidence=0.5, source="text_regex"
            ),
        }
