...
**Статус**: ✅ Учтено (2026-04-15)
**Проблема**: `tests/test_wave4_ai_contract.py` прошёл `black --check`, но не прошёл `isort --check-only` в CI. Локально `isort` был прогнан только на части файлов, новый файл был пропущен.
**Решение**: перед каждым коммитом прогонять `python -m isort --profile black --check-only src tests` — по всей директории, а не по отдельным файлам.
**Памятка**: `black` и `isort` — разные инструменты. `black` не сортирует импорты. Новые файлы легко пропустить если указывать файлы явно.
