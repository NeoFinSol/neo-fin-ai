...
- В персистентном контуре используется отдельный worker-процесс и Redis, поэтому длительный анализ не зависит от жизненного цикла одного HTTP-процесса
- Статусы обработки продолжают доходить до интерфейса через WebSocket благодаря мосту событий между worker-процессом и FastAPI
...
| `TASK_QUEUE_BROKER_URL` | — | Адрес Redis-брокера очереди задач для worker-процесса |
...
| `HF_TOKEN` | — | Токен HuggingFace API (DeepSeek) |
