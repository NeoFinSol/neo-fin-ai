...
Для локальной разработки можно использовать `docker-compose.override.yml` с безопасными дефолтными значениями:

```bash
# Скопируйте пример override-файла (ТОЛЬКО для локальной разработки!)
...
⚠️ **WARNING:** `docker-compose.override.yml` содержит слабые пароли (`postgres:postgres`) и должен использоваться **ТОЛЬКО** для локальной разработки. Никогда не используйте его в production, staging или CI/CD!

### Валидация окружения

Перед запуском проверьте корректность настройки окружения:

```bash
# Проверка переменных окружения
python scripts/validate_env.py

# Для локальной разработки (пропуск проверки)
