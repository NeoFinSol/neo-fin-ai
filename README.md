...
- **Smart Text Extraction**: автоматический выбор источника (PDF текст → OCR для сканов >50 страниц)
- **OCR Fallback**: для сложных PDF с оглавлением вместо данных → полное распознавание через Tesseract
...
- Показатели ниже порога `CONFIDENCE_THRESHOLD` (по умолчанию 0.5) автоматически исключаются из расчёта коэффициентов
- В UI каждый показатель маркируется цветом: 🟢 > 0.8 / 🟡 0.5–0.8 / 🔴 < 0.5
- Tooltip показывает метод извлечения и уровень уверенности
