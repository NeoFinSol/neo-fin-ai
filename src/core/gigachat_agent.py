        ...
        try:
            ...
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    logger.error(
                        "GigaChat auth failed: %d - %s", response.status, error_text
                    )
                    raise ValueError(
                        f"GigaChat authentication failed: {response.status}"
                    )
            ...
            try:
                ...
                async with session.post(
                    ...
                    if res.status != 200:
                        ...
                        logger.error(
                            "GigaChat API error %d: %s", res.status, error_text[:200]
                        )
                        ...
                        if attempt < retries - 1:
                            ...
                            logger.info(
                                "Retrying in %.2f seconds (status %d)...",
                                delay,
                                res.status,
                            )
                    ...
                    try:
                        ...
                        if choices and len(choices) > 0:
                            ...
                            if response:
                                logger.info(
                                    "Successfully received response from GigaChat"
                                )
                                return response
                        ...
                        text = await res.text()
                        logger.error(
                            "Unexpected content type from GigaChat API: %s", text[:200]
                        )
                        return text
                ...
                last_exception = e
                logger.exception(
                    "Unexpected error calling GigaChat API (attempt %d/%d): %s",
                    attempt + 1,
                    retries,
                    e,
                )
